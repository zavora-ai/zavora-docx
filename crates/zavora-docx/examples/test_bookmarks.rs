...
fn main() {
    ...
    // Some filler content
    let mut p = doc.add_paragraph("");
    p.add_run("This is section one with some content...")
        .font("Arial")
        .size(11.0);

    // Bookmarked paragraph
    let mut p = doc.add_paragraph("");
    p.add_run("Section Two - Bookmarked")
        .font("Arial")
        .size(14.0)
        .bold(true);
    ...
    let mut p = doc.add_paragraph("");
    p.add_run("This paragraph is the target of the internal link above.")
        .font("Arial")
        .size(11.0);

    doc.save("/Users/jameskaranja/Downloads/03_bookmarks.docx")
        .unwrap();
