...
fn main() {
    ...
    p.add_run("Advanced Styling Showcase")
        .font("Calibri Light")
        .size(28.0)
        .bold(true)
        .theme_color("accent1");
    ...
    p.add_run("Outline ")
        ...
        .size(18.0)
        .bold(true)
        .text_outline(0.5, "4472C4");
    p.add_run("Reflection")
        .font("Calibri")
        .size(18.0)
        .bold(true)
        .reflection();
    ...
    let mut p = doc.add_paragraph("");
    p = p.space_before(Length::pt(24.0));
    p.add_run("4. Table Conditional Formatting")
        .font("Calibri Light")
        .size(16.0)
        .bold(true);
    ...
    table = table
        .width_pct(100.0)
        .borders(BorderStyle::Single, 4, "BFBFBF");
    ...
    if let Some(mut c) = table.cell(0, 2) {
        c.set_text("Notes");
    }
    if let Some(mut c) = table.cell(1, 0) {
        c.set_text("Theme Colors");
    }
    if let Some(mut c) = table.cell(1, 1) {
        c.set_text("Complete");
    }
    if let Some(mut c) = table.cell(1, 2) {
        c.set_text("All 12 theme slots");
    }
    if let Some(mut c) = table.cell(2, 0) {
        c.set_text("Text Effects");
    }
    if let Some(mut c) = table.cell(2, 1) {
        c.set_text("Complete");
    }
    if let Some(mut c) = table.cell(2, 2) {
        c.set_text("Shadow, glow, outline, reflection");
    }
    if let Some(mut c) = table.cell(3, 0) {
        c.set_text("Drop Caps");
    }
    if let Some(mut c) = table.cell(3, 1) {
        c.set_text("Complete");
    }
    if let Some(mut c) = table.cell(3, 2) {
        c.set_text("Configurable line span");
    }
    ...
    if let Some(mut c) = table.cell(4, 2) {
        c.set_text("OpenType kerning + ligatures");
    }
    ...
    let mut p = doc.add_paragraph("");
    p = p.space_before(Length::pt(24.0));
    p.add_run("5. Kerning & Ligatures")
        .font("Calibri Light")
        .size(16.0)
        .bold(true);
    let mut p = doc.add_paragraph("");
    p.add_run("With kerning: ").font("Calibri").size(11.0);
    p.add_run("AVATAR WAV fly")
        .font("Georgia")
        .size(14.0)
        .kerning(8.0)
        .ligatures("standard");
    let mut p = doc.add_paragraph("");
    p.add_run("Fine typography with OpenType features enabled for professional output.")
        .font("Georgia")
        .size(11.0)
        .kerning(8.0)
        .ligatures("all");
    ...
    doc.save("/Users/jameskaranja/Downloads/09_advanced_styling.docx")
        .unwrap();
